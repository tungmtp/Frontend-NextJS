...
export default function ExtractBom() {
      ...
      for (i = 0; i < arrLength; i++) {
        if (extractBomData[i].bomLevel == level - 1) {
          console.log(
            ...
            maxDate,
            " timeOfDelay: ",
            extractBomData[i].timeOfDelay,
            " convert maxDate + timeOfDelay: ",
            today(maxDate + extractBomData[i].timeOfDelay * 1000 * 60 * 60 * 24)
          );
          if (!extractBomData[i].bomID) {
            extractBomData[i].dateFix = extractBomData[i].reqDate;
          } else {
            extractBomData[i].dateFix = selectMaxDate(
              today(maxDate.getTime() + timeOfDelay * 1000 * 60 * 60 * 24),
              extractBomData[i].dateFix
            );
          }
        }
      }
  ...
  const fixDate = (valFixDate) => {
    ...
    for (i = 0; i < arrLvlLength; i++) {
      ...
      const { minReqDate, maxReqDate, timeOfDelay } = extractBomData
        .filter((item) => item.bomLevel == reverseArrLevel[i])
        .reduce(
          (acc, xItem) => {
            if (xItem.dateFix) {
    ...
    const { maxDateFix } = extractBomData.reduce(
      (acc, item) => {
        ...
        if (dateMs > acc.maxDateFix) {
          acc.maxDateFix = dateMs;
        }
        return acc;
      },
      { maxDateFix: -Infinity }
    ...
    setMaxDateProduce(
      today(maxDateFix.getTime() + firstTimeOfDelay * 1000 * 60 * 60 * 24, "VN")
    );
  };
  ...
  const getExtractBomData = () => {
    ...
    asyncGetData(
      `/produce-service/bom/extract/${productId}/${measId}/${defaultDate}/${quantity}`
    )
        ...
        if (data) {
          ...
          asyncGetData(`/produce-service/bom/product/${productId}`)
            ...
            .then((data1) => {
              ...
              setMaxDateProduce(
                today(
                  maxDateDefault.getTime() +
                    data1[0].timeOfDelay * 1000 * 60 * 60 * 24,
                  "VN"
                )
              );
            })
  ...
  }, [productId, keyRender, defaultDate]);
        ...
        <FormControl fullWidth>
          <DatePicker
            ...
            onChange={(newValue) => {
              ...
              setDefaultDate(updatedDefaultDate);
            }}
          />
        </FormControl>
      </Grid>
      ...
      <Grid item xs={12}>
        <TableContainer component={Paper}>
          ...
          >
            ...
            <TableBody>
              {extractBomData
                .filter((item) => item.bomLevel <= showLevel)
                .map((item, index) => (
                  <TableRow hover key={index}>
                    ...
                    >
                      {item.reqDate.split("-").reverse().join("-")}
                    </TableCell>
                    <TableCell align="left" sx={{ border: 1 }}>
                      {item.segmentName}
                    </TableCell>
        ...
        >
          ...
          <DialogContent>
            <DialogContentText>
              Hãy gõ vào ngày vật tư hoặc bán thành phẩm này có thể có mặt tại
              nhà máy
            </DialogContentText>
            <TextField
              autoFocus
              ...
              value={(daySelected ? daySelected : today())
                ...
                .join("/")}
              onChange={(e) =>
                setDaySelected(e.target.value.split("/").reverse().join("-"))
              }
            />
          </DialogContent>
          <DialogActions>
